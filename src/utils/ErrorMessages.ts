...
export function handleValidation(error: CustomError, set: any) {
	set.status = 400;
	if (error.validator && error.type && error.validator.schema && error.validator.schema.properties) {
		return {
...
export function ErrorMessages(code: string, error: Error | CustomError, set: any) {
	switch (code) {
		case "NOT_FOUND":
